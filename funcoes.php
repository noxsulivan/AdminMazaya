...
$cores = array('FF6600','3399FF','009966','CC3399','FFCC33','6699CC','CC3366','123456','33FF66','FF6600');
  ...
  function twitpic($produto,$id_foto,$id){
	...
	function pdf($corpo,$nome_arquivo,$salvar_temp = FALSE){
...
//		$_POST['ese_prazo'] = $xml3->PrazoEntrega;
	...
	function mailClass($para,$subj,$msg,$from=NULL,$from_name=NULL,$arquivo=NULL,$html=true,$_para_nome = false){
			...
			if(!preg_match('/nox/i',$_SERVER['HTTP_HOST'])){
				...
				//	
//					
//					$_host = "smtp.mesacor.com.br";
//					$_port = 587;
//					$_user = "noreply@mesacor.com.br";
//					$_pass = 'noreply123';
					
					
					$_host = "smtp.gmail.com";
					$_port = 465;
					$_user = "contato@ngresinas.com.br";
					$_pass = '@82trmo836f';
					
					$_host = "smtp.gmail.com";
					$_port = 465;
					$_user = "site@mesacor.com.br";
					$_pass = 's1t02013';
				...
				foreach($_para as $__para){
					...
					//$__para = $matches[0][0];
					$m->AddTo($__para,(( $_para_nome ) ? $_para_nome : "Cliente"),'utf-8','base64',FALSE);
				}
				...
				$m->From( $from_email , $from_name,'utf-8','base64',FALSE);
				
				
				$m->AddHeader('Reply-To', $from_email,'utf-8','base64',FALSE);
				$m->AddHeader('Return-Path', "sulivan@ngresinas.com.br",'utf-8','base64',FALSE);
				...
				if($html){
					$corpo = '
						...
						<tr><td><div style="width:700px; font-family:Verdana, Geneva, sans-serif; color:#000; padding:50px; font-size:12px;">
								'.$msg.'
						</div></td></tr>
