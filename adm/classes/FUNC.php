							...
							class FUNC {
								...
								static public function trace($debug, $message = null, $level = 0, $ret = false) {
									if (self::is_debug($debug) && is_string($message) && ($level == 0 || $level == 1 || $level == 2)) {
										if ($level == 0) $mess = 'Erro';
										else if ($level == 1) $mess = 'Advertência';
										else if ($level == 2) $mess = 'Aviso';
										$emsg = '<b>'.$mess.'</b>: '.$message.
										...
										if ($level == 0) {
											if (defined('DISPLAY_XPM4_ERRORS') && DISPLAY_XPM4_ERRORS == true) echo $emsg;
											else exit;
								...
								static public function is_hostname($str = null, $addr = false, $debug = null) {
									...
									if (!is_bool($addr)) $err[] = 'invalid address type :"'.$addr.'"';
									if (count($err) > 0) self::trace($debug, implode(', ', $err));
									else {
										$ret = false;
								...
								static public function is_mail($addr = null, $vermx = false, $debug = null) {
									if (!self::is_debug($debug)) $debug = debug_backtrace();
									$err = array();
									if (!is_string($addr)) $err[] = 'Tipo de endereço inválido: "'.$addr.'"';
									...
									if (count($err) > 0) self::trace($debug, implode(', ', $err));
										...
										$ret = preg_match_all("/[0-9,a-z,A-Z,.,-,_]*@[a-z0-9\-]+\.([a-z0-9\-]+\.)*+[a-z]{2,3}/i",$addr,$matches);
										if ($ret && $vermx) {
