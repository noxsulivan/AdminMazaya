 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
					...
					class MIME {
						...
						static public function is_printable($str = null, $debug = null) {
							...
							if (!is_string($str)) FUNC::trace($debug, 'invalid argument type');
							else {
						...
						static public function encode_header($str = null, $charset = null, $encoding = null, $len = null, $end = null, $debug = null) {
							...
							if ($encoding == null) $encoding = self::HENCDEF;
								...
								$encoding = strtolower(FUNC::str_clear($encoding));
									...
									foreach (preg_split('/(?<!\\?(?i)q)\\?\\=/', $str, -1, PREG_SPLIT_NO_EMPTY) as $str1) {
										foreach (explode('=?', $str1, 2) as $str2) {
											...
											if (count($exp = explode('?B?', $str2)) == 2) {
												if (strlen($exp[0]) >= 2 && FUNC::is_alpha($exp[0], true, '-') && trim($exp[1]) != '') $def = array('charset' => $exp[0], 'value' => base64_decode(trim($exp[1])));
											} else if (count($exp = explode('?b?', $str2)) == 2) {
												if (strlen($exp[0]) >= 2 && FUNC::is_alpha($exp[0], true, '-') && trim($exp[1]) != '') $def = array('charset' => $exp[0], 'value' => base64_decode(trim($exp[1])));
											} else if (count($exp = explode('?Q?', $str2)) == 2) {
												if (strlen($exp[0]) >= 2 && FUNC::is_alpha($exp[0], true, '-') && $exp[1] != '') $def = array('charset' => $exp[0], 'value' => quoted_printable_decode(str_replace('_', ' ', $exp[1])));
											} else if (count($exp = explode('?q?', $str2)) == 2) {
												if (strlen($exp[0]) >= 2 && FUNC::is_alpha($exp[0], true, '-') && $exp[1] != '') $def = array('charset' => $exp[0], 'value' => quoted_printable_decode(str_replace('_', ' ', $exp[1])));
											}
											if ($def) {
						...
						static public function isset_header($str = null, $name = null, $value = null, $debug = null) {
							...
							if (!(is_string($name) && strlen($name) > 1 && FUNC::is_alpha($name, true, '-'))) $err[] = 'invalid name type';
						...
						static public function split_header($str = null, $debug = null) {
							if (!FUNC::is_debug($debug)) $debug = debug_backtrace();
							if (!(is_string($str) && $str != '')) FUNC::trace($debug, 'invalid header value');
							else {
								...
								$arr = array();
								foreach (explode("\n", $str) as $line) {
									$line = trim(FUNC::str_clear($line));
									if ($line != '') {
										if (count($exp1 = explode(':', $line, 2)) == 2) {
											$name = rtrim($exp1[0]);
											$val1 = ltrim($exp1[1]);
											if (strlen($name) > 1 && FUNC::is_alpha($name, true, '-') && $val1 != '') {
												$name = ucfirst($name);
												$hadd = array();
								...
								else FUNC::trace($debug, 'invalid header value', 1);
							}
						}
					
						static public function split_message($str = null, $debug = null) {
							if (!FUNC::is_debug($debug)) $debug = debug_backtrace();
							if (!(is_string($str) && $str != '')) FUNC::trace($debug, 'invalid message value');
							else {
								$ret = false;
								if (strpos($str, "\r\n\r\n")) $ret = explode("\r\n\r\n", $str, 2);
								else if (strpos($str, "\n\n")) $ret = explode("\n\n", $str, 2);
						...
						static private function _content($str = null, $debug = null) {
							if (!FUNC::is_debug($debug)) $debug = debug_backtrace();
							if (!(is_string($str) && $str != '')) FUNC::trace($debug, 'invalid content value');
							else {
