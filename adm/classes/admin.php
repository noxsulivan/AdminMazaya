...
function menu($idPai=0,$nivel=1){
		...
		if(count($usuario->tipos_de_usuarios->menus) > 0){
			foreach($usuario->tipos_de_usuarios->menus as $menu){
				...
				if($menu->propriedades['@menus'] == $idPai){
					...
					if($menu->fotos){
						$ret .= '<img src="'.$this->localhost.'_img/'.$menu->fotos[0]['id'].'/38.png" border="0" style="width:38px; height:38px" class="icone"><br> ';
					}
					$ret .= $menu->menu;
					$ret .= '</a>'.$sub.'</li>';
				}
...
function tit(){
		...
		$menu = new objetoDb("menus",$this->tg);
		
		//pre($menu);
		$titulo['icone'] = $this->localhost.'_img/'.$menu->fotos[0]['id'].'/42.png';
...
function barra(){
		...
		if(is_array($this->botoes_adicionais)){
			...
			$barra['botoes']['atualizar']['href'] = '#'.$this->view.'@'.$this->sub_tg.'@'.$this->sub_id.'';
		...
		if((@in_array($this->tabela,$usuario->tabelas) and !@preg_match('/^view_/i',$this->tabela))){
			...
			$barra['botoes']['novo']['href'] = '#'.$this->view.'@'.$this->sub_tg.'@'.$this->sub_id.'/novo/';
			$barra['botoes']['novo']['funcao'] = "formulario('".$this->view.'@'.$this->sub_tg.'@'.$this->sub_id."/novo')";
			$barra['botoes']['novo']['imagem'] = 'add.png';
		
			$barra['botoes']['deletar']['caption'] = 'Excluir';
			$barra['botoes']['deletar']['alt'] = 'Excluir selecionados';
			$barra['botoes']['deletar']['href'] = '#'.$this->view.'@'.$this->sub_tg.'@'.$this->sub_id;
			...
			$barra['botoes']['imprimir']['href'] = '#'.$this->view.'@'.$this->sub_tg.'@'.$this->sub_id;
			...
			if(preg_match("/=/i",$this->acao_busca)){/////////////filtros
				foreach($output as $campo=>$valor){
						if(preg_match("/_ini$/i",$campo)){
							...
							//$tmp['camposlistagem'][] = $this->view.".".$campo." like '%".$valor."%'";
						}
				}
...
//				}
				if( is_array($tmp['camposlistagem']) ){
					$_tmp = implode(" and ", $tmp['camposlistagem']);
					unset($tmp);
					$tmp['camposlistagem'][] = $_tmp;
				}
				...
				$lista["condicao"] = $usuario->condicao;
				if($usuario->condicao){
					$condicao = $usuario->condicao;
					
					if($db->campoExiste("idusuarios", $this->tabela) and preg_match("/__USUARIO_ID__/i",$condicao)) $camposCondicionais = preg_replace("/__USUARIO_ID__/i",$usuario->id,$condicao);
					if($db->campoExiste("idcadastros", $this->tabela) and preg_match("/__CADASTRO_ID__/i",$condicao)) $camposCondicionais = preg_replace("/__CADASTRO_ID__/i",$usuario->cadastros->id,$condicao);
					if($db->campoExiste("idcondominos", $this->tabela) and preg_match("/__CONDOMINO_ID__/i",$condicao) and $this->tabela != "condominos") $camposCondicionais = preg_replace("/__CONDOMINO_ID__/i",$usuario->condominos->id,$condicao);
					 //$this->campos[$campo]["Key"] == 'PRI'
					 
					 
					
					
					if($db->campoExiste("idclientes", $this->tabela) and preg_match("/__CLIENTE_ID__/i",$condicao)) $camposCondicionais = preg_replace("/__CLIENTE_ID__/i",$usuario->cadastros->id,$condicao);
					
				}
				//$sql = "select ".$this->view.".".$this->idtabela;
				$sql = "select ".$this->view.".*";
		...
		$rows = $db->rows;
			...
			foreach($this->campos_listagem as $campo => $valor){
				...
				if(preg_match('/>/i',$valor)){
					...
					$camposCondicionais = NULL;
						if($usuario->condicao){
							$condicao = $usuario->condicao;
							
							if($db->campoExiste("idusuarios", $this->tabela) and preg_match("/__USUARIO_ID__/i",$condicao)) $camposCondicionais = preg_replace("/__USUARIO_ID__/i",$usuario->id,$condicao);
							if($db->campoExiste("idcadastros", $this->tabela) and preg_match("/__CADASTRO_ID__/i",$condicao)) $camposCondicionais = preg_replace("/__CADASTRO_ID__/i",$usuario->cadastros->id,$condicao);
							if($db->campoExiste("idcondominos", $this->tabela) and preg_match("/__CONDOMINO_ID__/i",$condicao)) $camposCondicionais = preg_replace("/__CONDOMINO_ID__/i",$usuario->condominos->id,$condicao);
					...
					if(preg_match("/\->/i",$tmp1)){
						...
						$sql = "select a.id".$tmp1.",a.".$primeiro." from ".$tmp1." as a,".$this->tabela." as b where a.id".$tmp1." = b.id".$tmp1.($tmp1 == $this->tabela ? "_2": "").($camposCondicionais ? " and (a.".$camposCondicionais.")" : "")." group by  a.id".$tmp1." order by a.".$primeiro."";
					}
					...
					if($db->rows < 200){
						while($res = $db->fetch()){
							...
							}elseif($tmp1 == 'condominos'){
								$obj = new objetoDb($tmp1,$res["id".$tmp1]);//pre($obj);
								$header['opcoes'][$res["id".$tmp1]] = trim(str_pad($obj->quadras->quadra,2,"0",STR_PAD_LEFT)."-".str_pad($obj->lote,2,"0",STR_PAD_LEFT)." ".substr($obj->$primeiro,0,30));
							}else{
					...
					$header['classe'] = 'f_'.diretorio(preg_replace("/\(.*\)/i","",$this->campos[$valor]['Type'])).'';
					$header['campo'] =$valor;
					$opcoes = explode("','",str_replace("')",'',str_replace("enum('",'',$this->campos[$valor]["Type"])));
					$_op = 0;
					foreach($opcoes as $op)
							$header['opcoes'][$op] = $op;
				}elseif(preg_match('/tinyint\(1\)/i',$this->campos[$valor]['Type'])){
					$header['classe'] = 'f_'.diretorio(preg_replace("/\(.*\)/i","",$this->campos[$valor]['Type'])).'';
					$header['campo'] =$valor;
				...
				switch(true){
					case 'varchar(15)' == $this->campos[$valor]["Type"]:
					...
					case 'tinyint(4) unsigned' == $this->campos[$valor]["Type"]:
					...
					break;
					case 'tinyint(1)' == $this->campos[$valor]["Type"]:
			...
			if($db->campoExiste("id".$this->tabela,"arquivos")){
				$header['href'] = $this->localhost.'download/';
				$header['classe'] = 'f_link';
			...
			if($this->campos_aditivos){
						foreach($this->campos_aditivos as $campo => $valor){
							$header['visor'] = $campo;
							$header['classe'] = 'f_aditivos';
							$header['campo'] = 'arquivo';
							$lista['headers'][] = $header;
						}
			}
						...
						if(is_object($obj->$campo)){
							if($this->tabela == $campo){
								...
								if($extrasubcampo){
									...
									if($campo == "condominos" and $subcampo == "nome"){
										$visor = trim(str_pad($obj->$campo->quadras->quadra,2,"0",STR_PAD_LEFT)."-".str_pad($obj->$campo->lote,2,"0",STR_PAD_LEFT)." ".substr($obj->$campo->nome,0,30));
									}else{
										$visor = $obj->$campo->$subcampo;
									}
								}
		...
		//pre(__LINE__);pre($usuario->tabelas);die();
...
//					}
					...
					if($this->campos_aditivos){
						foreach($this->campos_aditivos as $campo => $valor){
							include("includes/_clientes/aditivo_".$valor.".php");
						}
					}
					$lista['itens'] = $item;
				}
			 ...
			 $lista['itens'] = array();
						...
						if(file_exists("includes/_clientes/relatorio_".$this->tabela.".php")){
								
								include("includes/_clientes/relatorio_".$this->tabela.".php");
