				...
				case 'fotos':
						...
						if($db->campoExiste('id'.$this->tabela,'fotos')){
							if($this->tabela != 'fotos'){
								...
								$db->query("select idfotos,legenda,nome_arquivo,dimensoes,width,height from fotos where id".$this->tabela." = '".$this->id."' order by ordem asc");
								while($res = $db->fetch()){
									if($res['height'] < 1){
					...
					if($db->tabelaExiste($campo)){
						if($db->indiceExiste($campo,$this->tabela) and isset($this->propriedades["@".$campo])){//////// se é um objeto pai e seu indice está apontado
							...
							//pre('aqui 2'.$this->tabela.$campo);
								$_subResource = $db->resourceAtual;
								$primeiroCampo = $db->primeiroCampo($campo);
							$db->query('select id'.$campo.' from '.$campo.' where id'.$this->tabela.' = "'.$this->id.'" order by '.$primeiroCampo);
							//pre($db);
							while($res = $db->fetch()){
							...
							while($res = $db->fetch()){
								$_ordem = '';
								$objeto = new objetoDb($campo,$res['id'.$campo]);
								if($objeto->ordem){
									$_campo[$objeto->ordem] = $objeto;
									$_ordem = $objeto->ordem;
								}else{
									$_campo[++$j] = $objeto;
								}
