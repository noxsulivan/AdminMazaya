 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
					...
					class SMTP {
						...
						static private function _cres($conn = null, &$resp, $code1 = null, $code2 = null, $debug = null) {
							...
							if (count($err) > 0) return FUNC::trace($debug, implode(', ', $err), 1);
							else {
								$ret = true;
								do {
						...
						static public function mxconnect($host = null, $port = null, $tout = null, $name = null, $context = null, $debug = null) {
							...
							$_RESULT = array();
							if (!FUNC::is_debug($debug)) $debug = debug_backtrace();
							if (!is_string($host)) FUNC::trace($debug, 'invalid host type');
							else {
								$host = strtolower(trim($host));
								if (!($host != '' && FUNC::is_hostname($host, true, $debug))) FUNC::trace($debug, 'invalid host value');
							}
							$res = FUNC::is_win() ? FUNC::getmxrr_win($host, $arr, $debug) : getmxrr($host, $arr);
							$con = false;
							if ($res) {
								foreach ($arr as $mx) {
						...
						static public function connect($host = null, $port = null, $user = null, $pass = null, $vssl = null, $tout = null, $name = null, $context = null, $login = null, $debug = null) {
							...
							if ($user != null) {
								...
								else if (($user = FUNC::str_clear($user)) == '') $err[] = 'invalid username value';
							}
							if ($pass != null) {
								if (!is_string($pass)) $err[] = 'invalid password type';
								else if (($pass = FUNC::str_clear($pass)) == '') $err[] = 'invalid password value';
							}
							...
							if ($name != null) {
								if (!is_string($name)) $err[] = 'invalid name type';
									...
									if (!($name != '' && ($name == 'localhost' || FUNC::is_ipv4($name) || FUNC::is_hostname($name, true, $debug)))) $err[] = 'invalid name value';
								}
							} else $name = '127.0.0.1';
						...
						static public function send($conn = null, $addrs = null, $mess = null, $from = null, $debug = null) {
							...
							global $_RESULT;
							$_RESULT = $err = array();
							if (!is_resource($conn)) $err[] = 'Conexão com servidor inválida';
							if (!is_array($addrs)) $err[] = 'Tipo de endereço inválido para "'.$addrs.'"';
							else {
								$aver = true;
								if (count($addrs) > 0) {
									foreach ($addrs as $addr) {
										if (!FUNC::is_mail($addr)) {
											$aver = false;
											break;
										}
									}
								} else $aver = false;
								if (!$aver) $err[] = 'Endereço de e-mail inválido "'.print_r($addrs,true).'"/"'.$addr.'"';
							}
							if (!is_string($mess)) $err[] = 'Corpo da mensagem em branco ou impróprio';
							if ($from == null) {
								$from = @ini_get('sendmail_from');
								if ($from == '' || !FUNC::is_mail($from)) $from = (isset($_SERVER['SERVER_ADMIN']) && FUNC::is_mail($_SERVER['SERVER_ADMIN'])) ? $_SERVER['SERVER_ADMIN'] : 'postmaster@localhost';
							} else {
								if (!is_string($from)) $err[] = 'Tipo do e-mail remetente é inválido';
								else if (!($from != '' && FUNC::is_mail($from))) $err[] = 'E-mail do remetente é inválido';
							}
							if (count($err) > 0) FUNC::trace($debug, implode(', ', $err));
							else {
								$ret = false;
						...
						static public function disconnect($conn = null, $debug = null) {
							...
							global $_RESULT;
							$_RESULT = array();
							if (!is_resource($conn)) return FUNC::trace($debug, 'Conexão com servidor inválida', 1);
							else {
						...
						static public function quit($conn = null, $debug = null) {
							...
							$_RESULT = array();
							$ret = false;
							if (!is_resource($conn)) FUNC::trace($debug, 'Conexão com servidor inválida');
						...
						static public function ehlo($conn = null, $host = null, $debug = null) {
							...
							$_RESULT = $err = array();
							if (!is_resource($conn)) $err[] = 'Conexão com servidor inválida';
							if (!is_string($host)) $err[] = 'Tipo de host inválido';
							else {
								$host = strtolower(trim($host));
								if (!($host != '' && ($host == 'localhost' || FUNC::is_ipv4($host) || FUNC::is_hostname($host, true, $debug)))) $err[] = 'invalid host value';
							}
						...
						static public function from($conn = null, $addr = null, $debug = null) {
							...
							else if (!($addr != '' && FUNC::is_mail($addr))) $err[] = 'invalid from address value';
							if (count($err) > 0) FUNC::trace($debug, implode(', ', $err));
							else {
								$ret = false;
						...
						static public function to($conn = null, $addr = null, $debug = null) {
							...
							else if (!($addr != '' && FUNC::is_mail($addr))) $err[] = 'invalid to address value';
							if (count($err) > 0) FUNC::trace($debug, implode(', ', $err));
							else {
								$ret = false;
						...
						static public function rset($conn = null, $debug = null) {
							...
							$_RESULT = array();
							$ret = false;
							if (!is_resource($conn)) FUNC::trace($debug, 'invalid resource connection');
