...
case "novo";
	...
	if($db->campoExiste('idprodutos','chas')){
...
default:
	...
	$admin->titulo = " de chas. Noivos: ".$_cli->nome_noiva." ".$_cli->sobrenome_noiva." & ".$_cli->nome_noivo." ".$_cli->sobrenome_noivo;
	
	switch($admin->sub_tg){
		case "noivas":
			$sql = "select * from chas where idclientes = '".$admin->sub_id."'" ;
		...
		default:
			...
			$sql = "select * from chas".( $usuario->clientes->id ? " where idclientes = '".$usuario->clientes->id."'" : ($admin->sub_tg ? " where idclientes = '".$admin->sub_tg."'" : "" )) ;
		break;
	}
