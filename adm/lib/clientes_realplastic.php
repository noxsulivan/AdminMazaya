...
case "novo";
	...
	if($usuario->tipos_de_usuarios->id == 1){
		...
		if($db->campoExiste('idplanos','clientes')){
			...
			//if($db->campoExiste('blog','clientes'))
			$formulario->fieldset->simples('Código de acesso', 'codigo_acesso');
			$formulario->fieldset->separador();
		...
		if($db->tabelaExiste('contatos')){
			$formulario->fieldset('Histórico de consumo para os último 6 meses');
				$formulario->fieldset->grafico('clientes_consumo');
		}
		...
		if($db->tabelaExiste('contatos')){
			...
			$formulario->fieldset("Produtos consumidos");
			$formulario->fieldset->filhos("materiais_consumidos");
		...
		if($db->tabelaExiste('presentes') and !$db->campoExiste('idplanos','clientes')){
		...
		if($db->campoExiste('idclientes','fotos')){
			$formulario->fieldset("Evento");
				$formulario->fieldset->simples('Mensagem dos noivos', 'mensagem_noivos');
				$formulario->fieldset->simples('Local do evento - Escreva o endereço no formato padrão e teste no GooleMpas para conferir', 'local_evento');
...
//		if($db->tabelaExiste('clientes_has_produtos')){
//			$formulario->fieldset("Grades consumidos");
//			$formulario->fieldset->checkBox("Grades consumidos","produtos");
//		}
...
case "salvar":
	...
	if($admin->id == ''){
		...
		$db->tabela_link('clientes','materiais',$inserted_id,$_POST["materiais"]);
		$db->tabela_link('clientes','produtos',$inserted_id,$_POST["produtos"]);
		...
		$db->tabela_link('clientes','materiais',$admin->id,$_POST["materiais"]);
		$db->tabela_link('clientes','produtos',$admin->id,$_POST["produtos"]);
		
		
		if($db->campoExiste('idclientes','arquivos')){
			$db->query("select * from arquivos where idclientes = '".$admin->id."'");
			while($res = $db->fetch()){
				copy($res['path'],"../_arquivos/".$res['arquivo']);
			}
		}
