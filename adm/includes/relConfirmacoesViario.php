...
$cliente = new objetoDb('clientes',$_id);
		...
		$ret .= "<h1><br>Relat&oacute;rio de Confirmação de Presença</h1>";
...
//		4 - fulano3 - (id boleto) B 2 115,00 12/08/09 13/08/09 Q 
		...
		$db->query('select idconfirmacoes from confirmacoes where idclientes="'.$cliente->id.'" order by '.($ordem ? $ordem : 'nome').' '.$sentido);
		  ...
		  if($db->rows){
				...
				while($res = $db->fetch()){
					...
					$ret .= '
					<tr>
						<td colspan="9"><strong>Nenhuma confirmação</strong></td>
					</tr>';
		  }
		...
		$ret .= '<h2>Total: '.( $totaladultos + $totalcriancas).' - Adultos: '.$totaladultos.' - Crianças: '.$totalcriancas.'</h2>';
